...
def sidebar_controls(dataset_label: str) -> Dict[str, object]:
    ...
    if meta["dimensions"]:
        composition_dim = st.sidebar.selectbox("Dimensión de composición", meta["dimensions"], index=0)
    else:
        composition_dim = None
        st.sidebar.info("La base seleccionada no trae dimensiones de composición adicionales.")
    compare_scope = st.sidebar.radio("Comparación", ["Comuna vs región", "Comuna vs país"], index=0)
    active_view = st.sidebar.radio(
        "Vista activa",
        ["Resumen ejecutivo", "Evolución temporal", "Composición", "Benchmark", "Calidad y datos"],
        index=0,
    )
    ...
    extra_filters: Dict[str, List[str]] = {}
    if meta["dimensions"]:
        eligible_filter_dims = []
        for dim in meta["dimensions"]:
            nunique = int(df[dim].dropna().nunique()) if dim in df.columns else 0
            if 0 < nunique <= 60:
                eligible_filter_dims.append(dim)
        selected_filter_dims = st.sidebar.multiselect(
            "Activar filtros por dimensión",
            options=eligible_filter_dims,
            default=[],
        )
...
# TABS
# =========================================================
def render_summary_tab(df_filtered: pd.DataFrame, metric: str, composition_dim: Optional[str]) -> None:
    ...
    with right:
        if composition_dim and composition_dim in df_filtered.columns:
            ...
            st.markdown('<div class="soft-card">Esta base no incluye una dimensión categórica adicional para ranking de composición.</div>', unsafe_allow_html=True)
...
def render_timeseries_tab(df_filtered: pd.DataFrame, metric: str, composition_dim: Optional[str]) -> None:
...
def render_composition_tab(df_filtered: pd.DataFrame, metric: str, composition_dim: Optional[str]) -> None:
    st.markdown('<div class="section-title">Estructura y composición</div>', unsafe_allow_html=True)

    if not composition_dim or composition_dim not in df_filtered.columns:
        st.info("La base seleccionada no tiene una dimensión de composición adicional para esta vista.")
        return
    ...
    if composition_dim in df_filtered.columns and "Año" in df_filtered.columns and not rank_table.empty:
        ...
        if not share_df.empty:
            ...
            if not latest_top.empty and share_df[composition_dim].nunique() <= 20:
...
def main() -> None:
    ...
    try:
        with st.spinner("Cargando base y preparando visualización..."):
    ...
    view = controls.get("active_view", "Resumen ejecutivo")
